...
const NotepadApp = () => {
  ...
  const addNote = async () => {
    try {
      ...
      if (response.status === 201) {
        ...
        setNewNote('');
        setSuccessMessage('Nota salva corretamente');
  ...
  const deleteNote = async noteId => {
    try {
      ...
      if (response.status === 200) {
        ...
        setNotes(updatedNotes);
        setOriginalNotes(updatedNotes);
        setSuccessMessage('Nota excluída corretamente');
  ...
  return (
    <Container>
      ...
      <Header as="h1" className="notepad-title">Bloco de anotações</Header>
      <div className="note-input-container">
        ...
        />
        <Button onClick={addNote} className="add-note-button green">Salvar</Button>
      </div>
      <h3>Filtros:</h3>
